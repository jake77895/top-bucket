module CareerDataShared
  ...
  included do
    if self.name == "SiteController"
      before_action :initialize_mappings, only: [:home]
    elsif self.name == "CareerDataController"
      before_action :initialize_mappings, only: [:show, :node_data]
    end
  end
  ...
  def node_data
    ...
    jobs = CombinedJob.where(
      "LOWER(industry) = ? AND LOWER(level) = ?",
